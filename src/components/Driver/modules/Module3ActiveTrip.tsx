    ...
    <div className="space-y-4 font-sans">
      ...
      <div className="bg-card border border-border p-4 rounded shadow-sm">
        ...
        <div className="mb-3">
          ...
          <div className="grid grid-cols-2 gap-2 mt-2 text-xs font-mono">
            <div className="p-2 bg-secondary/50 rounded border border-border">
              ...
              <strong className="text-foreground text-sm">{distanceToStop} meters</strong>
            </div>
            <div className="p-2 bg-secondary/50 rounded border border-border">
              <span className="text-[10px] text-muted-foreground block">Estimated Arrival</span>
              <strong className="text-foreground text-sm">{etaMinutes} mins</strong>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between pt-2 border-t border-border text-xs font-mono text-muted-foreground">
          <span>Speed: <strong className="text-foreground">{telemetry?.speedKmH || 32} km/h</strong></span>
          <span>Heading: <strong className="text-foreground">{Math.round(telemetry?.heading || 90)}°</strong></span>
          <span>Progress: <strong className="text-foreground">{Math.round((telemetry?.progressAlongRoute || 0.4) * 100)}%</strong></span>
        </div>
      </div>
      ...
      <div className="bg-card border border-border p-4 rounded shadow-sm">
        <div className="flex items-center justify-between pb-2 mb-3 border-b border-border">
          ...
          <span className="text-[10px] font-mono text-muted-foreground">
            {isDwellActive ? `Dwell Timer: ${dwellSeconds}s` : 'Transit Protocol'}
          </span>
        </div>

        {isDwellActive ? (
          <div className="space-y-3">
            <div className="p-3 bg-secondary/60 border border-foreground/30 rounded text-xs font-mono space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2 text-[11px] pt-1">
                <div className="p-1.5 bg-card rounded border border-border">
                  <span className="text-muted-foreground block">Boarding (In)</span>
                  <strong className="text-foreground">+{boardingCount} Pax</strong>
                </div>
                <div className="p-1.5 bg-card rounded border border-border">
                  <span className="text-muted-foreground block">Alighting (Out)</span>
            ...
            <button
              ...
              className="w-full py-2 bg-foreground text-background font-mono text-xs font-bold uppercase rounded hover:opacity-90 transition-opacity cursor-pointer flex items-center justify-center gap-2"
            >
          ...
          <div className="space-y-2">
            <p className="text-xs text-muted-foreground leading-relaxed">
              Tap below when reaching {nextStopName} to log dwell time and passenger boarding count.
            </p>
            <button
              onClick={() => setIsDwellActive(true)}
              className="w-full py-2 bg-secondary hover:bg-accent text-foreground border border-border font-mono text-xs font-bold uppercase rounded transition-colors cursor-pointer flex items-center justify-center gap-2"
            >
      ...
      <div className="bg-card border border-border p-4 rounded shadow-sm">
        ...
        {/* Progress Bar */}
        <div className="w-full h-2 bg-secondary rounded overflow-hidden mb-3">
          <div
        ...
        {isHighOccupancy && (
          <div className="mt-3 p-2 bg-destructive/10 border border-destructive/30 rounded text-destructive text-xs font-mono flex items-center gap-2">
            <AlertTriangle className="w-4 h-4 shrink-0" />
            <span>Capacity &gt;90%. Eligible for Module 4 passenger overflow assistance.</span>
          </div>
        )}
      </div>
      ...
      {route && (
        <div className="bg-card border border-border p-4 rounded shadow-sm text-xs font-mono">
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              ...
              <strong className="text-foreground">{route.code} — {route.name}</strong>
            </div>
            <div>
